...
Memo.prototype.sinc_comunica = function () {
    ...
    var post = "memo_cambs=" + encodeURIComponent(JSON.stringify(this.sinc_stato.camb_aspettanti));
    var ultimo_update = this.sinc_stato.ultimo_update || 0;
    var url = "/memo/api/sinc.php?db=" + this.nome_db + "&ultimo_update=" + ultimo_update;
    Memo.ajax(url, post).then(function (responseText) {
        ...
        if (this.sinc.fetch_interval > this.sinc.max_fetch_interval) {
            this.sinc.fetch_interval = this.sinc.max_fetch_interval;
        }
        if (this.sinc.fetch_interval < this.sinc.min_fetch_interval) {
            this.sinc.fetch_interval = this.sinc.min_fetch_interval;
        }
        var righe = [], i;
